...
Defaultwerte (s. L</Dokument/Eigenschaften>).
...
L</Anker>). Wenn falsch, unterbleibt dies.
...
L</Inhaltsverzeichnis/Eigenschaften>):
...
(siehe L</"Unterabschnitte von Inhaltsverzeichnis ausschließen">).
...
Eine Zwischenüberschrift wird wie ein L</Abschnitt> vereinbart,
...
Für Grafiken, die im Textfluss erscheinen sollen, siehe Segment L</G>.
...
Wie bei einer Tabelle (s. L</Tabelle>) steht am Anfang und am Ende der
...
Wird nichts anderes angegeben (siehe L</"Formatierte Inhalte inkludieren">), wird Sdoc-Text inkludiert.
...
Jeder Abschnitt (s. L</Abschnitt>) und jedes Element einer
Definitionsliste (s. L</Definitionsliste>) erhält automatisch
einen oder mehrere Anker zugewiesen (sofern als Dokumenteigenschaft
nicht L</generateAnchors>=0 definiert ist):
...
Anker bzw. Schlüssel-Anker (s. Segment L</A> oder L</K>)
...
A und a erreicht (s. Segment L</A>).
...
Inline-Grafiken können im Gegensatz zu Abbildungen (s. L</Abbildung>)
nicht betitelt und nicht durch Links (s. Segment L</L>)
referenziert werden.
...
Link1: L</Inline-Segment/L> (Titel-Pfad als Link-Text)

Link2: L</L> (Titel-Pfad als Link-Text)

Link3: L</Inline-Segment/L> (Anker als Link-Text)

Link4: L</L> (Anker als Link-Text)

Link5: L</L> (eigener Text als Link-Text)
