...
=head3 statistic() - Information über Daten einer Relation oder Selektion

=head4 Synopsis

    $h = $db->statistic(@select);
...
sub statistic {
    ...
    my ($row) = $self->select($stmt);

    my $titleMaxLen = 0;
    my %len = map {$_ => 0} @suffixes;
    $len{'rowcount'} = length $row->rowcount;
    for my $title (@$titleA) {
        my $l = length $title;
        if ($l > $titleMaxLen) {
            $titleMaxLen = $l;
        ...
        for my $suffix (@suffixes) {
            my $l = length $row->get($title.'_'.$suffix);
            if ($l > $len{$suffix}) {
                $len{$suffix} = $l;
            }
        }
    }
    $len{'minval'} = -$len{'minval'};
    $len{'maxval'} = -$len{'maxval'};

    printf "%s\n",$row->rowcount;
    for my $title (@$titleA) {
        printf '%*s:',$titleMaxLen,$title;
        for my $suffix (@suffixes) {
            printf ' %*s',$len{$suffix},$row->get($title.'_'.$suffix);
        }
        print "\n";
    }
